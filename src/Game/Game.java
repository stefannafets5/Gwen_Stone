...
public class Game {
    ...
    public void cardUsesAttack(Coordinates cardAttacker, Coordinates cardAttacked, ConvertJson out) {
        ...
        if (y2 < getBoard().get(x2).size()) {
            ...
            if (playerIdx == 0 && (x2 == 2 || x2 == 3)) {
                out.friendlyFire(cardAttacker, cardAttacked);
            } else if (playerIdx == 1 && (x2 == 0 || x2 == 1)) {
                out.friendlyFire(cardAttacker, cardAttacked);
            } else if (getBoard().get(x1).get(y1).getHasAttacked() == 1) {
                out.hasAttacked(cardAttacker, cardAttacked);
            } else if (getBoard().get(x1).get(y1).getIsFrozen() == 1) {
                out.isFrozen(cardAttacker, cardAttacked);
            } else if (enemyHasTank == 1 && attackedCardIsTank == 0) {
                out.attackedCardNotTank(cardAttacker, cardAttacked);
            } else { // can attack
